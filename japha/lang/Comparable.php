 ...
 * used as keys in a sorted map or elements in a sorted set, without the need to specify a 
 * comparator.
 * 
 * The natural ordering for a class C is said to be consistent with equals if and only 
 * if (e1.compareTo((Object)e2) == 0) has the same boolean value as e1.equals((Object)e2) for 
 ...
 * inconsistent with equals. In particular, such a sorted set (or sorted map) violates 
 * the general contract for set (or map), which is defined in terms of the equals method.
 *
 ...
 * false (and the size of the sorted set does not increase) because a and b are equivalent 
 ...
 * ordering equates BigDecimal objects with equal values and different precisions 
 ...
 * relation on C, and that the natural ordering is a total order on C. When we say that a 
 * class's natural ordering is consistent with equals, we mean that the quotient for the 
 * natural ordering is the equivalence relation defined by the class's equals(Object) method:
 *
...
interface Comparable
{
	 ...
	 * signum function, which is defined to return one of -1, 0, or 1 according to whether 
	 * the value of expression is negative, zero or positive. The implementor must 
	 * ensure sgn(x.compareTo(y)) == -sgn(y.compareTo(x)) for all x and y. 
	 * (This implies that x.compareTo(y) must throw an exception iff y.compareTo(x) throws 
	 * an exception.)
	 *
	 ...
	 *
	 * Generally speaking, any class that implements the Comparable interface and violates this 
